...
# Function to continuously get logs
def get_logs_continuously(stop_event, log_lines, update_event):
    while not stop_event.is_set():
        new_logs = get_logs()
        if new_logs:
            log_lines["logs"] = new_logs
            update_event.set()  # Signal that logs have been updated
        time.sleep(1)

def get_logs():
    """Gets the logs from the queue."""
    logs = []
    while True:
        try:
...
async def run_browser_agent(
    ...
    # Clear the log queue before starting a new task
...
def create_ui(theme_name="Ocean"):
    ...
    with gr.Blocks(title="Browser Use WebUI", theme=theme_map[theme_name], css=css, js=js) as demo:
        log_lines = gr.State({"logs": "", "dummy": 0})
        stop_event = threading.Event()
        update_event = threading.Event()

        background_thread = threading.Thread(
            target=get_logs_continuously,
            args=(stop_event, log_lines, update_event)
        )
        ...
        logger_output.change(
            fn=update_logs,
            inputs=[log_lines],
            outputs=[logger_output]
        )
